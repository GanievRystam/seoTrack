...
function normalizeMetricsSnapshot(value) {
    const source = typeof value === "object" && value !== null ? value : {};
    const parseMetrics = (key) => {
...
function getPageStatus(metrics) {
...
function normalizeAdditionalPage(value) {
    ...
    return {
        ...
        status: page.status === "warning" || page.status === "error" || page.status === "ok"
            ? page.status
...
function normalizeAdditionalPages(value) {
    if (!Array.isArray(value))
        return [];
    return value
...
async function runChecksForPage(url) {
...
function publishSse(projectId, payload) {
...
async function runCheckForProject(project, runId) {
    try {
        ...
        const checkedAdditionalPages = await Promise.all(enabledAdditionalPages.map(async (page) => {
        ...
        const totalAlerts = countAlerts(mainPageResult.metrics.mob) +
            countAlerts(mainPageResult.metrics.desc) +
            checkedAdditionalPages.reduce((sum, page) => sum + countAlerts(page.metrics.mob) + countAlerts(page.metrics.desc), 0);
...
async function enqueueCheckRun(project) {
...
app.patch("/projects/:id/settings", requireAuth, async (req, res) => {
    const id = getSingleParam(req.params.id);
    const { name, url, checkFrequency } = req.body;
...
app.get("/projects/:projectId/check-runs", requireAuth, async (req, res) => {
    ...
    try {
        ...
        const normalized = runs.map((r) => {
            const metricsSource = r.metrics?.rawJson;
            const scriptsSource = r.scripts?.scripts;
            ...
            const mainScripts = Array.isArray(scriptsSource)
                ? normalizeScripts(scriptsSource)
                : normalizeScripts(typeof scriptsSource === "object" && scriptsSource !== null ? scriptsSource.mainPage : []);
            return {
                id: r.id,
...
app.get("/incidents", requireAuth, async (req, res) => {
    try {
        ...
        const priority = (lvl) => (lvl === "critical" ? 2 : 1);
        incidents.sort((a, b) => {
